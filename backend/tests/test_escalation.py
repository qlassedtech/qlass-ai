...
def test_hint_only_turns_build_up_the_streak(db_session):
    student = _make_student(db_session)
    for _ in range(ESCALATION_THRESHOLD - 1):
        assert record_hint_outcome(student, solved_directly=False) is False
    assert record_hint_outcome(student, solved_directly=False) is True


def test_direct_solve_resets_the_streak(db_session):
    student = _make_student(db_session)
    record_hint_outcome(student, solved_directly=False)
    record_hint_outcome(student, solved_directly=False)
    record_hint_outcome(student, solved_directly=True)
    assert student.consecutive_unresolved_hints == 0
    # Needs a full fresh streak after the reset, not just one more hint.
    assert record_hint_outcome(student, solved_directly=False) is False


def test_non_problem_turn_does_not_affect_streak(db_session):
    student = _make_student(db_session)
    record_hint_outcome(student, solved_directly=False)
    record_hint_outcome(student, solved_directly=None)
