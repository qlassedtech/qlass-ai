...
_SYSTEM_PROMPT = (
    ...
    "hint_mode — this is about changing a STICKY SETTING for all future messages, not about "
    "the current problem. on ONLY if the student is explicitly asking to turn ON that setting "
    "going forward (e.g. \"turn on hint mode\", \"enable hint mode\", \"hint mode on\", \"don't "
    "just give me answers [from now on]\", \"stop solving it for me, just give hints [going "
    "forward]\"). off ONLY if explicitly asking to turn it OFF going forward (e.g. \"turn off "
    "hint mode\", \"hint mode off\", \"disable hint mode\", \"just give me the answers again "
    "[from now on]\"). A student simply asking for a hint on the problem in front of them right "
    "now — e.g. \"give me a hint\", \"can I get a hint\", \"hint please\", \"I'm stuck, help\" — "
    "is NOT a settings change and must be none; that is an ordinary tutoring request, handled "
    "like any other message, not a hint_mode toggle. none for every other message — this is "
    "rare, default to none.\n\n"
