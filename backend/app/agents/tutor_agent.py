...
class TutorAgent(BaseAgent):
    ...
    def build_context(self, student: dict, retrieved_chunks: list[str]) -> str:
        profile = (
            ...
            f"a separate translation step converts it into the student's own language afterward, so "
            f"just focus on getting the English content and teaching right. This translation step is "
            f"an internal implementation detail — NEVER mention, explain, or reassure the student about "
            f"it (e.g. don't say things like \"I'll type in English but it gets converted\"). As far as "
            f"the student is concerned, you're just answering directly — write naturally, with zero "
            f"reference to English, translation, or how the pipeline works.\n"
