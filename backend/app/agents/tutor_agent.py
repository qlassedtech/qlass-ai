    ...
    def build_context(
        ...
        profile = (
            ...
            + (
                ...
                "diagram/drawing/mind map/concept map/summary map, OR a diagram is genuinely "
                "necessary to explain THIS reply's new content (e.g. labeled parts of a cell, a "
                "circuit, the water cycle) — not for every explanation, most questions don't need "
                "one. CRITICAL — false whenever this reply is just evaluating/grading an answer to a "
                "check question you asked (evaluated=true above), even if an earlier turn on the same "
                "topic included an image: a student answering \"Rectum\" to your own question isn't "
                "asking for another diagram, and correcting their answer in words doesn't need one "
                "either — don't regenerate an image just because the topic already had one earlier in "
                "the conversation. false otherwise. When image=true, add ONE more line directly "
                "BEFORE the TRACK line, in this exact format: [[IMAGE_PROMPT: <a clear, specific "
                "description for an image generator, e.g. \"a labeled diagram of a plant cell showing "
                "nucleus, chloroplast, mitochondria, vacuole, for a Class 8 science textbook\">]]. For "
                "any science/math concept, the prompt must explicitly ask for a clearly labeled "
                "educational diagram (labeled parts/axes/steps, simple clean line-art style, plain "
            ...
            + (
                ...
                "confuses a student more than no video at all). If THIS reply has no new teaching content "
                "of its own (e.g. the student just asked for a video on a topic already explained earlier "
                "in the conversation, and your reply is only a short acknowledgment like \"Sure, here's a "
                "good one!\"), the query MUST be the topic from that EARLIER explanation, verbatim — never "
                "invent a different topic just because this turn's visible text is short (confirmed live: "
                "a student who asked \"what is cctv\" then, on the next turn, just \"send me a video\", got "
                "a video about something else entirely — the query must have been \"CCTV\" both times, not "
                "re-derived from the near-empty acknowledgment text). Your visible reply text must still stand "
